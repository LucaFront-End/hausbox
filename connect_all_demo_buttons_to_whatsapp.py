...
for filepath in html_files:
    ...
    # 1. Update <a href="..." class="btn-cta-nav ..."> in nav-pills
    # Replace href="#" and remove open-calc-btn
    pattern = r'<a\s+href="[^"]*"\s+class="btn-cta-nav group[^"]*"[^>]*>'
    new_tag = f'<a href="{WA_URL}" class="btn-cta-nav group" target="_blank" rel="noopener noreferrer">'
    
    if re.search(pattern, content):
        content = re.sub(pattern, new_tag, content)
        # Also ensure SVG inside btn-cta-nav is WA_SVG
        content = re.sub(
            r'(<a\s+href="' + re.escape(WA_URL) + r'"\s+class="btn-cta-nav group"[^>]*>)\s*<svg[\s\S]*?</svg>',
            f'\\1\n          {WA_SVG}',
            content
        )
    ...
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(content)
    updated += 1
    print(f"[OK] Updated nav btn-cta-nav to WhatsApp in {fname}")

print("Completed updating all nav demo buttons to WhatsApp.")
